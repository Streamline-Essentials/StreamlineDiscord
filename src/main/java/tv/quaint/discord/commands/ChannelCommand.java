            ...
            case "remove":
                if (messagedString.getCommandArgs().length == 1) {
                    ...
                    DiscordHandler.getLoadedChanneledFolders().forEach((string, folderRemove) -> {
                        folderRemove.getAssociatedRoutes(EndPointType.DISCORD_TEXT, messagedString.getChannel().getId()).forEach(route -> {
                ...
                DiscordHandler.getLoadedChanneledFolders().forEach((string, folderRemove) -> {
                    routes.addAll(folderRemove.getBackAndForthRoute(typeRemove, identifier, messagedString.getChannel().getId()));
                });
